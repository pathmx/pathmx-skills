...
export async function inspectDrift(layout: TargetLayout, canonicalSkills = skillsRoot) {
  ...
  const canonicalNames = new Set(layout.skills)
  const targetEntries = await readdir(layout.agentSkillsDir, {
    withFileTypes: true,
  }).catch(() => [])
  for (const entry of targetEntries) {
    if (!canonicalNames.has(entry.name)) {
      drift.push({ kind: "extra", path: `${entry.name} (non-canonical)` })
    }
  }
  ...
  if (!(await linkMatches(layout.claudeSkills, layout.agentSkillsDir))) {
    ...
    const value = current?.isSymbolicLink()
      ? await readlink(layout.claudeSkills)
      : current
        ? "non-link"
        : "missing"
...
export async function writeSkills(
  ...
  const claudeBackup = path.join(transaction, "backup-claude-skills")
  await mkdir(stageRoot, { recursive: true })

  let backedUpSkills = false
  let installedSkills = false
  let backedUpClaudeSkills = false
  let createdClaudeLink = false
  let createdClaudeDir = false

  async function rollback() {
    if (createdClaudeLink) await removeEntry(layout.claudeSkills)
    if (backedUpClaudeSkills) await rename(claudeBackup, layout.claudeSkills)
    if (installedSkills) await removeEntry(layout.agentSkillsDir)
    if (backedUpSkills) await rename(skillBackupRoot, layout.agentSkillsDir)
  ...
  try {
    ...
    if (await stats(layout.agentSkillsDir)) {
      await rename(layout.agentSkillsDir, skillBackupRoot)
      backedUpSkills = true
    }
    await rename(stageRoot, layout.agentSkillsDir)
    ...
    if (await stats(layout.claudeSkills)) {
      await rename(layout.claudeSkills, claudeBackup)
      backedUpClaudeSkills = true
    }
    await symlink(
      expectedLink(layout.claudeSkills, layout.agentSkillsDir),
      layout.claudeSkills,
    )
    createdClaudeLink = true
